MAPJS.DOMRender =
  ...
  dimensionProvider: (idea, level) ->
    'use strict'
    textBox = $('.chemistry').nodeWithId(idea.id)
    ...
    result =
      width: textBox.outerWidth(true) + 12
      height: textBox.outerHeight(true) # - 17
    textBox.detach()
    result
...
$.fn.updateNodeContent = (nodeContent, resourceTranslator) ->
  ...
  textSpan = ->
    ...
    if span.length == 0
      span = $('<span>').attr('data-mapjs-role', 'title').appendTo(self)
    span
  ...
  setColors = ->
    fromStyle = nodeContent.attr and nodeContent.attr.style and nodeContent.attr.style.background
    if fromStyle == 'false' or fromStyle == 'transparent' or !fromStyle
      fromStyle = $('atom-text-editor').css('background-color')
      if fromStyle == 'rgba(0, 0, 0, 0)'
        fromStyle = 'rgba(255,255,255,.4)'
  ...
  setStyle = ->
    style = nodeContent.attr and nodeContent.attr.style
    if style
      keys = Object.keys(style)
      for key in keys
        self.css key, style[key]
  ...
  self.attr 'nodeId', nodeContent.id
  self.attr 'edge', (nodeContent.attr and nodeContent.attr.edge)
  self.attr 'type', (nodeContent.attr and nodeContent.attr.type)
  self.attr 'path', (nodeContent.attr and nodeContent.attr.scope)
  self.attr 'scope', (nodeContent.attr and nodeContent.attr.scope)
...
MAPJS.DOMRender.viewController = (mapModel, stageElement, touchEnabled, imageInsertController, resourceTranslator, options) ->
  ...
  ensureNodeVisible = (domElement) ->
    ...
    if !_.isEmpty(animation)
      viewPort.animate animation, duration: 0 #CHEMISTRY 100
    return
  ...
  mapModel.addEventListener 'nodeCreated', (node) ->
    ...
    element = stageElement.createNode(node).queueFadeIn(nodeAnimOptions).updateNodeContent(node, resourceTranslator).on('tap', (evt) ->
      ...
      return
    ).each(ensureSpaceForNode).each(updateScreenCoordinates).on('mapjs-start-dragging mapjs-start-dragging-shadow', ->
  ...
  mapModel.addEventListener 'linkCreated', (l) ->
    ...
    link.find('.mapjs-link-hit').on 'tap', (event) ->
      ...
      event.gesture.stopPropagation()
      return
    stageElement.nodeWithId(l.ideaIdFrom).add(stageElement.nodeWithId(l.ideaIdTo)).on('mapjs-move mapjs-drag', ->
      link.updateLink()
      return
    ).on 'mapjs-animatemove', ->
  ...
  mapModel.addEventListener 'linkAttrChanged', (l) ->
    attr = _.extend({ arrow: false }, l.attr and l.attr.style)
