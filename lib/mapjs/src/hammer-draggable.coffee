do ->
  ...
  $.fn.simpleDraggableContainer = ->
    ...
    drag = (event) ->
      if currentDragObject and event.gesture
        ...
        currentDragObject.css(newpos).trigger $.Event('mapjs-drag',
    ...
    rollback = (e) ->
      ...
      if target.attr('mapjs-drag-role') != 'shadow'
        target.animate originalDragObjectPosition,
          complete: ->
            target.trigger $.Event('mapjs-cancel-dragging', gesture: e.gesture)
            return
          progress: ->
            target.trigger 'mapjs-drag'
            return
      else
        target.trigger $.Event('mapjs-cancel-dragging', gesture: e.gesture)
      return
    ...
    @on('mapjs-start-dragging', (event) ->
      if !currentDragObject
        ...
        $(this).on 'drag', drag
      return
    ).on('mapjs-start-dragging-shadow', (event) ->
      ...
      if !currentDragObject
        ...
        originalDragObjectPosition =
          top: currentDragObject.css('top')
          left: currentDragObject.css('left')
        currentDragObject.on('mapjs-stop-dragging mapjs-cancel-dragging', (e) ->
          @remove()
          e.stopPropagation()
  ...
  onDrag = (e) ->
    $(this).trigger $.Event('mapjs-start-dragging',
      relatedTarget: this
      gesture: e.gesture)
    e.stopPropagation()
    e.preventDefault()
  ...
  onShadowDrag = (e) ->
    $(this).trigger $.Event('mapjs-start-dragging-shadow',
      relatedTarget: this
      gesture: e.gesture)
    e.stopPropagation()
    e.preventDefault()
