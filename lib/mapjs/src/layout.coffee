...
MAPJS.Outline = (topBorder, bottomBorder) ->
  'use strict'
  #console.log 'MAPJS.outline(topBorder,bottomBorder)', topBorder, bottomBorder
...
MAPJS.Tree = (options) ->
  'use strict'
  #console.log 'MAPJS.Tree: ', options
...
MAPJS.calculateLayout = (idea, dimensionProvider, margin) ->
  ...
  setDefaultStyles = (nodes) ->
    _.each nodes, (node) ->
      ...
      node.attr.style = _.extend({}, MAPJS.defaultStyles[if node.level == 1 then 'root' else 'nonRoot'], node.attr.style)
      return
    return
