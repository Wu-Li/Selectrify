MAPJS.MapModel = (layoutCalculatorArg, selectAllTitles, clipboardProvider, defaultReorderMargin) ->
  ...
  @getStyleForId = (id, prop) ->
    ...
    node and node.style and node.style[prop]
  ...
  @scale = (source, scaleMultiplier, zoomPoint) ->
    if isInputEnabled
      ...
      analytic (if scaleMultiplier < 1 then 'scaleDown' else 'scaleUp'), source
    return
  ...
  self.pasteStyle = (source) ->
    ...
    if isInputEnabled and clipContents and clipContents[0]
      pastingStyle = clipContents[0].attr and clipContents[0].style
      self.applyToActivated (id) ->
  ...
  self.toggleEdges = () ->
    if $(".toggleEdges")[0].checked
      $("#laboratory").addClass('show-edges')
    else
      $("#laboratory").removeClass('show-edges')

  self.toggleIDs = () ->
    if $(".toggleIDs")[0].checked
      $("#laboratory").addClass('show-ids')
    else
      $("#laboratory").removeClass('show-ids')

  self.toggleTypes = () ->
    if $(".toggleTypes")[0].checked
      $("#laboratory").addClass('show-types')
    else
      $("#laboratory").removeClass('show-types')
