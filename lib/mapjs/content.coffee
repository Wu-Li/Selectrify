MAPJS.content = (contentAggregate, sessionKey) ->
  ...
  contentAggregate.canUndo = ->
    ! !(eventStacks[sessionKey] and eventStacks[sessionKey].length > 0)

  contentAggregate.canRedo = ->
    ! !(redoStacks[sessionKey] and redoStacks[sessionKey].length > 0)
